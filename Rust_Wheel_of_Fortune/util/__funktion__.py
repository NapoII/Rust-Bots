...
class Region:
    """
Detects a click within a given region.

Args:
    regions (dict): A dictionary of regions with their respective coordinates and dimensions.
        Each key is a region name, and the corresponding value is a `Region` object with four attributes:
...
Returns:
    str: The name of the region that was clicked, or None if no region was clicked.

Example Usage:
    >>> r1 = Region(10, 10, 100, 100)
    >>> r2 = Region(150, 10, 100, 100)
    >>> regions = {'Region 1': r1, 'Region 2': r2}
    >>> detect_click_in_regions(regions)
    # Click inside Region 1
    'Region 1'
"""
...
def detect_click_in_regions(regions):
    while True:
        clicked_region = None

        def on_click(x, y, button, pressed):
            nonlocal clicked_region
            for name, region in regions.items():
                if pressed and region.x <= x <= region.x + region.width and region.y <= y <= region.y + region.height:
                    clicked_region = name
                    break  # Stop checking regions

            return clicked_region is None  # Stop the listener if a region has been clicked
